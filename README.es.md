...
| v2.7.0 | Sesiones que se deslizan en lugar de caducar a mitad de tarea — una sesión activa se renueva a la mitad de su ventana y, cuando llega a agotarse, lo dice y te devuelve al inicio de sesión en vez de fallar una petición en silencio. Espacios y los listados de facturación pasan al mismo aspecto de lista y detalle que ya usa el resto de la aplicación, y se traducen las últimas pantallas que quedaban solo en inglés. **Autoalojamiento:** una versión se despliega en un VPS que no tiene git ni copia del código; las guías de instalación y actualización descargan un tarball de la publicación en lugar de clonar; y una primera instalación indica qué fichero guarda las claves que se perderían con el servidor, con `scripts/pull-backup.sh` para copiarlas fuera | Hecho |
| v2.7.1 | Una primera instalación deja ahora un `POST-INSTALL.md` en el servidor con lo que el instalador no puede hacer por ti — copiar `.env` fuera del host, crear la organización y el super administrador, programar copias de seguridad y sacarlas de la máquina, configurar el correo antes de que se descarte en silencio — con todas las rutas ya rellenadas. Un primer despliegue repite lo primero de esa lista en la cabecera de la ejecución. El aviso de la v2.7.0 era correcto y nadie lo leyó: aparecía en la línea 248 de un registro de 1.150 líneas, dentro de un paso plegado. **Además:** los ejemplos de uso de `pull-backup.sh` ya no llevan un alias de host ni un puerto SSH reales | Hecho |
