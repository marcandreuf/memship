...
| v2.5.0 | Tot el correu sortint fa servir una única plantilla amb la identitat de l'organització — nom, logotip i color es llegeixen de la configuració, cada missatge inclou ara una alternativa en text pla al costat de l'HTML, i l'enviament de rebuts i el resum de facturació recurrent passen a ser plantilles en lloc d'HTML incrustat. A més: una inscripció el rebut de la qual falla en generar-se ja no es desfà amb ell, i `SEED_EMAIL_DOMAIN` permet que una instal·lació que envia correu real sembri un club de demostració en un domini capaç de rebre'l | Fet |
| v2.6.0 | Un super administrador decideix quins correus envia el sistema — una pantalla de Configuració amb un interruptor per plantilla, i els 17 punts d'enviament passant per una única porta en lloc d'enviar sense condicions. L'indicador de la funció de comunicats ara tanca els seus endpoints en comptes de només amagar la navegació, i una política de plantilles il·legible falla tancada, perquè no equival a un consentiment. **Canvi incompatible:** el correu opcional queda desactivat per defecte, de manera que una instal·lació existent es queda en silenci en actualitzar fins que un super administrador activi les plantilles que vulgui — només la verificació d'adreça i el restabliment de contrasenya continuen enviant-se | Fet |
| v2.7.0 | Sessions que llisquen en lloc de caducar a mig treball — una sessió activa es renova a la meitat de la seva finestra i, quan s'acaba, ho diu i et torna a l'inici de sessió en comptes de fer fallar una petició en silenci. Espais i els llistats de facturació passen al mateix aspecte de llista i detall que ja fa servir la resta de l'aplicació, i es tradueixen les darreres pantalles que quedaven només en anglès. **Autoallotjament:** una versió es desplega en un VPS que no té git ni còpia del codi; les guies d'instal·lació i actualització descarreguen un tarball de la publicació en lloc de clonar; i una primera instal·lació indica quin fitxer guarda les claus que es perdrien amb el servidor, amb `scripts/pull-backup.sh` per copiar-les fora | Fet |
| v2.7.1 | Una primera instal·lació deixa ara un `POST-INSTALL.md` al servidor amb allò que l'instal·lador no pot fer per tu — copiar `.env` fora de l'amfitrió, crear l'organització i el super administrador, programar còpies de seguretat i treure-les de la màquina, configurar el correu abans que es descarti en silenci — amb tots els camins ja emplenats. Un primer desplegament repeteix el primer d'aquests punts a la capçalera de l'execució. L'avís de la v2.7.0 era correcte i ningú no el va llegir: apareixia a la línia 248 d'un registre de 1.150 línies, dins d'un pas plegat. **A més:** els exemples d'ús de `pull-backup.sh` ja no porten un àlies d'amfitrió ni un port SSH reals | Fet |
